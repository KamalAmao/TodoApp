...
public class SearchWords {
    public int getNumberOfCharacterOccurence(String word, char search){
        int counter = 0;
        int len = word.length();
        for(int c = 0; c < len; c++){
            if(word.charAt(c) == search){
                counter++;
            }
        }
        return counter;
    ...
    public static void main(String[] args) {
        SearchWords myWord = new SearchWords();
        int numberOfOccurence = myWord.getNumberOfCharacterOccurence("Betty butter bought some butter", 'b'); 
        System.out.println("The numbers of occurence is " +numberOfOccurence);
